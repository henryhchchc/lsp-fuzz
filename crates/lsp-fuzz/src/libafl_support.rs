#![allow(
    dead_code,
    reason = "Some adapters are used only by optional fuzzing configurations."
)]
...
impl<T> OptionExt<T> for Option<T> {
        ...
        F: FnOnce() -> Result<T, E>,
    {
        if let Some(value) = self {
            ...
            let value = generator()?;
            *self = Some(value);
            // SAFETY: We just inserted a value, so it is safe to unwrap.
...
impl<L: HasLen, R: HasLen> HasLen for EitherTuple<L, R> {
    fn len(&self) -> usize {
        match self {
...
impl<L: NamedTuple, R: NamedTuple> NamedTuple for EitherTuple<L, R> {
