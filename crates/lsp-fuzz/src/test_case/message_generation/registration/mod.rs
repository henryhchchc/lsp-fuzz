...
use crate::{
    lsp::{LspMessage, LspMessageMeta, MessageParam},
    test_case::{
        LspInput,
        message_generation::{
            GenerationError, GeneratorsConfig, HasGenerators, LspParamsGenerator,
        },
    },
};
    ...
    M::Params: HasGenerators<State> + MessageParam<M>,
{
    fn mutate(
        ...
        let params = match generator.generate(state, input) {
            ...
            Err(GenerationError::Error(err)) => return Err(err),
        };
